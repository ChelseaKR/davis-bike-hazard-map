...
### EXP-07 — Versioned open-data snapshots + machine-readable catalog metadata ✅ DONE
**Pitch:** make the export a citable dataset, not just a live endpoint.

> **Shipped:** dated, checksummed, ODbL GeoJSON snapshots + a DCAT/schema.org
> `Dataset` catalog (`server/lib/snapshots.ts`, scheduler in `server/index.ts`),
> served CORS-`*` at `GET /api/exports`, `/api/exports/catalog.jsonld`, and
> `/api/exports/YYYY-MM-DD.geojson[.sha256]`. Snapshots regenerate from current
> data each run so reporter deletions propagate to previously published dates
> (trade-off documented + tested). See `docs/OPEN-DATA.md`; `CITATION.cff`
> extended with a dataset reference; tests in `tests/unit/snapshots.test.ts`.
