...
**Status: DONE (2026-07-02)** — advisory pid lock file (`{path}.lock`, created
with flag `wx`) acquired in `JsonFileRepository`'s constructor before `load()`
(`server/lib/repository.ts`); a second process fails loudly with a clear
error, stale locks (dead pid, reused/own pid, unparsable content) are
reclaimed with one retry, and `close()` plus a `process.on('exit')` hook
release the lock. Tests in `tests/unit/repository.lock.test.ts`.
