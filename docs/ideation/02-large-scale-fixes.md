...
## FIX-12 — Verify and document the real fuzzing guarantee — DONE
**Pitch:** property-test `fuzzCoordinate` and reconcile the "cell centre" comment with the code's cell-edge behavior.

> **Done** (branch `roadmap/fix-12-property-test-and-precisely-docum-29`):
> kept the cell-edge behaviour (switching to centres would shift every
> already-published point one cell — no-migration option chosen; trade-off
> documented in the `snap`/`fuzzCoordinate` JSDoc). fast-check property tests
> in `tests/unit/geo.test.ts` enforce displacement < 1.5 x grid (105 m;
> measured worst case ~= 99 m ~= sqrt(2) x 70 m), determinism, same-latitude
> same-cell indistinguishability, re-fuzz boundedness, and 6-decimal output.
> `privacy-notes.md` now quotes the test-enforced figure.
