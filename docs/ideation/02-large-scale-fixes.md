...
**Status: DONE (2026-07-02)** — merged `i18n-catalog-retrofit` and
`research-panel-and-roadmap` into one line. The research features (R1 dedupe
nudge, R2 reporter feedback trail, R4 coverage normalization, plus
RESEARCH-ROADMAP.md / USER-RESEARCH.md / `src/lib/dedupe.ts` /
`src/lib/reportTrail.ts` / `normalizeCoverage` / the `GET /api/reports/:clientId`
trail endpoint) now sit behind the react-intl catalog: the conflicting strings
in `ReportForm.tsx`, `MyReports.tsx`, and `CoverageView.tsx` were rewrapped in
`FormattedMessage`/`intl.formatMessage`, the new `near_miss` category and trail
labels were added to the catalog, and the full `npm run verify` battery
(lint + lint:css + i18n:gates + typecheck + test:unit + build) is green.
