...
## FIX-05 — Delta feed for mobile data — DONE (2026-07-02)
**Pitch:** an `updatedSince` cursor so the 30-second poll ships only what changed.

> **Done** on branch `roadmap/fix-05-updatedsince-delta-feed-for-mobil`:
> `GET /api/hazards?updatedSince=<epoch-ms>` returns
> `{hazards (changed only), deletedIds, serverTime}`; deletions surface as
> id-only tombstones (`hazard_tombstones` table via `0004_delta_feed.sql`,
> pruned by `expire()`), stale cursors fall back to the full feed, and
> `useHazards` merges deltas by id. Covered by `tests/unit/deltaFeed.test.ts`.
> Built without FIX-04 (added its own `listUpdatedSince`/`listTombstones`
