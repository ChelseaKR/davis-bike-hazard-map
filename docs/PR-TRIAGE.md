...
are no neutral, skipped or canceled conclusions to explain away.
...
`.github/codeql-acknowledged.json`. The gate fails on a "stale" acknowledgment,
meaning one that matches no error-level finding in the SARIF it was handed. An
acknowledgment for a JavaScript finding therefore matches nothing during the
...
::error::CodeQL gate failed with 1 problem(s):
  - stale acknowledgment: js/user-controlled-bypass at server/app.ts matched no
    error-level finding.
EXIT=1
```

Zero findings, and the gate still fails. The acknowledgment list can never be
...
dedicated new tests fail on reversion with behavioral assertions, not import
...
whose behavior genuinely does not change. The other 4, in
...
change routes: 400 by day against 800 at night straddles a 600 meter detour, so
...
### The generated doc audit serializes the entire queue
...
1. **Scope acknowledgments to a language, or run the gate once over all SARIFs.**
   As written, the stale-acknowledgment guard and the per-language matrix cannot
...
1. **The gate-falsifiability work is not on `main`.** Commits `180501d`,
   ...
   `origin/main` is still `540a5c8`. Every staleness judgment here is measured
...
- **#146's cross-language acknowledgment bug**, reproduced locally by running
  `scripts/codeql-gate.mjs` against a hand-built zero-result SARIF with the
  committed acknowledgment list. Exit 1 on zero findings.
...
- **#142's test discrimination**, by reverting the production fix and confirming
  the new test fails.
- **#145's counting logic, reversion behavior and mutation coverage**, and the
...
- **That the two `actions/cache-poisoning/poisonable-step` findings in
  `release.yml` are true positives.** CodeQL's judgment is reported as-is; the
  release workflow's checkout of `needs.authorize.outputs.release-commit` was not
  independently analyzed for exploitability.
