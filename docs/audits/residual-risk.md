...
| R3 | **Info disclosure** — precise location de-anonymizes a reporter | ~70 m deterministic fuzzing; precise point server-only | Low — fuzz grid is a tunable trade-off (`DEFAULT_FUZZ_METERS`) | privacy reviewer |
...
| R8 | **Elevation/Tampering** — `GET /api/hazards` formerly published each hazard's `clientId`, which doubles as the deletion capability for `DELETE /api/reports/:clientId` — anyone could scrape the feed and silently delete any report (found + fixed 2026-07-02, FIX-01) | `clientId` removed from the public `Hazard` projection (`server/lib/hazards.ts` `toPublic`) and the `shared/types.ts` `Hazard` interface; it now lives only on `StoredHazard` and the reporter's own device. Regression test "never leaks the reporter clientId in any unauthenticated response (FIX-01)" (`tests/unit/server.test.ts`) asserts the feed, export, and create/confirm bodies never carry it | **Medium until rotated, then Low.** The fix stops *future* publication only: any `clientId` served by a pre-fix deployment remains a valid deletion proof (`DELETE /api/reports/:clientId` cannot tell a reporter from a feed-scraper). Operator action, once per pre-fix deployment: rotate the stored ids of rows created before the fixed build went live — Postgres: `UPDATE hazards SET client_id = gen_random_uuid() WHERE created_at < <fix-deploy-epoch-ms>;` JSON dev store: rewrite the `clientId` fields in `DATABASE_PATH` (or accept the risk for throwaway dev data). Trade-off: reporters lose self-delete for pre-rotation reports (moderators can still remove them). If the public feed was never reachable before the fixed build, no ids were burned and rotation is a no-op — record which case applied and when. Ongoing: audit any new projection for capability-ish fields before exposing it | maintainer |
