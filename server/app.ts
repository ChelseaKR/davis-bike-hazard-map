...
export async function buildApp(deps: AppDeps): Promise<FastifyInstance> {
  ...
  app.get('/api/hazards/export', async (_req, reply) => {
    ...
    return reply
      ...
      .send(JSON.stringify(exportFeatureCollection(hazards)));
  });

  // --- Versioned open-data snapshots + DCAT/schema.org catalog (EXP-07) ---
  // Dated, checksummed, ODbL snapshots written to disk by the snapshot
  // scheduler (server/lib/snapshots.ts). All served CORS '*' as open data.
  const snapshotDir = config.snapshot?.dir ?? '';

  // List available snapshot dates + the catalog/self URLs.
  app.get('/api/exports', async (_req, reply) => {
    reply.header('access-control-allow-origin', '*');
    const dates = snapshotDir && existsSync(snapshotDir)
      ? readdirSync(snapshotDir)
          .map((f) => parseSnapshotName(f))
          .filter((p): p is { date: string; checksum: boolean } => p !== null && !p.checksum)
          .map((p) => p.date)
          .sort()
      : [];
    return reply.send({
      license: SNAPSHOT_LICENSE,
      catalog: '/api/exports/catalog.jsonld',
      snapshots: dates.map((date) => ({
        date,
        geojson: `/api/exports/${date}.geojson`,
        sha256: `/api/exports/${date}.geojson.sha256`,
      })),
    });
  });

  // The machine-readable Dataset catalog (JSON-LD).
  app.get('/api/exports/catalog.jsonld', async (_req, reply) => {
    reply.header('access-control-allow-origin', '*');
    const file = snapshotDir ? join(snapshotDir, 'catalog.jsonld') : '';
    if (!file || !existsSync(file)) {
      return reply.status(404).send({ error: 'not_found', message: 'No catalog yet.' });
    }
    return reply
  ...
  // A single dated snapshot (.geojson) or its checksum (.geojson.sha256).
  app.get('/api/exports/:name', async (req, reply) => {
    reply.header('access-control-allow-origin', '*');
    const { name } = req.params as { name: string };
    const parsed = parseSnapshotName(name);
    if (!parsed || !snapshotDir) {
      return reply.status(404).send({ error: 'not_found', message: 'No such snapshot.' });
    }
    ...
    return reply
      .header('content-type', parsed.checksum ? 'text/plain; charset=utf-8' : 'application/geo+json')
      .send(readFileSync(file, 'utf8'));
  });
